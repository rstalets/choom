...
def test_version_falls_back_to_0_0_0_without_version_file(monkeypatch) -> None:
    ...
    monkeypatch.delitem(sys.modules, "choom", raising=False)
    monkeypatch.delitem(sys.modules, "choom._version", raising=False)

    real_import = builtins.__import__

    def fake_import(name: str, *args: object, **kwargs: object) -> object:
        if name == "choom._version":
            raise ImportError(name)
        return real_import(name, *args, **kwargs)

    monkeypatch.setattr(builtins, "__import__", fake_import)

    module = importlib.import_module("choom")
    try:
        assert module.__version__ == "0.0.0"
    finally:
        monkeypatch.delitem(sys.modules, "choom", raising=False)
        importlib.import_module("choom")
