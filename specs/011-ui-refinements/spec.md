...
**Sequenced after**: `010-read-on-load` (issue #51), which has landed on main and retired the
session-lifetime snapshot. This spec is written for the codebase as it now stands — see Dependencies and
Relationships for what that settles.
...
- **A background re-read lands while a confirmation is on screen**: the confirmation stays up, keeps its
  ...
  refreshed list as it is. This holds whether or not the confirmation suspends the list underneath it.
- **The record is deleted by another process between the confirmation appearing and being confirmed**:
  the delete reports that the record no longer exists and the list refreshes — the same outcome as a row
  that was already stale when `ctrl+d` was pressed.
...
- **There is no session-lifetime cache.** `010-read-on-load` has landed, so a delete does not have to
  invalidate a snapshot, evict a record, or tell any view that something is gone — the next load reads
  the files. The list refresh FR-011 requires is for immediate feedback, not for correctness, and it is a
  re-read of what is on disk.
...
- **Issue #51 (read from disk on view load)**, specified as `010-read-on-load`, **has landed on main**
  and is merged into this branch. It retired the session-lifetime snapshot: every list load, and every
  return to a list from another screen, reads the workspace files at that moment, and a displayed list
  re-reads itself on a short interval. Three consequences for this spec, each verified against the merged
  code:
  - There is no in-memory cache for a delete to invalidate. The per-document and per-task snapshots are
    gone, as are the paths that used to repair them after a write. FR-011's refresh is a re-read, not
    cache surgery, and no code path needs to notify a view that a record has gone.
  - A periodic re-read can land while a confirmation is on screen. The refresh pauses whenever the list
    screen is suspended, so a confirmation that takes over the screen stops it; a confirmation drawn over
    a live list does not. FR-010 covers both by requiring the confirmation to act on the record it named
    when raised, so no background re-read can redirect a delete onto a different record.
  - Nothing this feature touches was changed by it. The top bar, the status bar, and the confirmation
    dialog are untouched by `010-read-on-load`, so Stories 1, 5, and 6 land on the same surfaces they
    were specified against. Only the delete stories meet its code, and they meet it as a simplification.
