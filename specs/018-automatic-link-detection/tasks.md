...
- [ ] T002 Reproduce the `_LINK_RE` probe from the table above in a throwaway snippet against
...
- [ ] T003 Add the `UrlConversion` frozen slotted dataclass to `src/choom/core/models.py` per
...
- [ ] T004 **Reuse `_mask_fences` and `_mask_code_spans` verbatim — write no new code for either**
...
- [ ] T005 [P] Add `_mask_frontmatter` to `src/choom/core/links.py` — blank from a leading `---\n` to
...
- [ ] T006 [P] Add `_mask_comments` to `src/choom/core/links.py` — blank `<!--` through the next
...
- [ ] T007 **Add `_mask_links` to `src/choom/core/links.py` — the task where this feature is most
...
- [ ] T008 [P] Add `_mask_angle` to `src/choom/core/links.py` — blank `<…>` runs containing no
...
- [ ] T009 [P] Add `_mask_refdefs` to `src/choom/core/links.py` — blank a link reference definition,
...
- [ ] T010 Compose the seven masks into one private pipeline in `src/choom/core/links.py`, in this
...
- [ ] T011 [US1] Add the candidate scanner to `src/choom/core/links.py`: the pattern
...
- [ ] T012 [US1] Add the emitter to `src/choom/core/links.py`: build `[{url}]({destination})` where
...
- [ ] T013 [US1] Assemble the public `format_bare_urls(text) -> tuple[str, tuple[UrlConversion, ...]]`
      ...
      `uv run mypy src`
- [ ] T014 [US1] Export `format_bare_urls` and `UrlConversion` from `src/choom/core/__init__.py`'s
...
- [ ] T015 [US1] Wire the conversion into `save_buffer` in `src/choom/core/editing.py`. **The order is
...
- [ ] T016 [US1] Add `map_cursor_offset(conversions, offset) -> int` to `src/choom/core/links.py` per
...
- [ ] T017 [US1] Use the mapping in `EditorPane._save` in `src/choom/tui/edit_screen.py`. The buffer
...
- [ ] T018 [US1] Add the status message in `src/choom/tui/edit_screen.py` per
...
- [ ] T019 [US1] Add the second save site: `open_task_editor._save` in `src/choom/tui/edit_screen.py`
...
- [ ] T020 [US2] Add the trailing-boundary trim to `src/choom/core/links.py`: repeatedly drop a final
...
- [ ] T021 [US2] Encode **all 25 rows** of Corpus A from
...
- [ ] T022 [US2] Encode **all 18 rows** of Corpus B from
      [contracts/text-format.md](./contracts/text-format.md) as a table-driven test in
...
- [ ] T023 [US2] Encode **all 15 rows** of Corpus C (adversarial) from
...
- [ ] T024 [US2] **The most important test in this feature.** Assert three-pass idempotency in
...
- [ ] T025 [US2] **Pin the newline-count invariant** in `tests/unit/test_bare_url_format.py`: for a
...
- [ ] T026 [US2] Add the whole-document integration assertions to
...
- [ ] T027 [US3] Confirm the boundary rows of Corpus A are all present and passing from T021 — a URL
...
- [ ] T028 [US3] Add a regression case that would have caught the bug the prototype actually hit:
...
- [ ] T029 [US4] Add to `tests/integration/test_bare_url_save.py`: **opening** a document whose body
...
- [ ] T030 [US4] [P] Add to `tests/integration/test_bare_url_save.py`: `choom links heal` and
...
- [ ] T031 [US4] [P] Add to `tests/integration/test_bare_url_save.py`: `check_links` reports an
...
- [ ] T032 [US4] [P] Add to `tests/integration/test_bare_url_save.py`: a task description containing a
...
- [ ] T033 [US4] [P] Add to `tests/integration/test_bare_url_save.py`: `choom note new` and
...
- [ ] T034 [P] Confirm the preview pane is unaffected: a converted link still opens in the browser,
      ...
      the diff
- [ ] T035 [P] Confirm no other documentation needs amending: `docs/REQUIREMENTS.md` is unchanged
...
- [ ] T036 **Leave README.md alone — this is a deliberate skip, not an oversight.** Per CLAUDE.md the
      ...
      `git diff`
- [ ] T037 Verify cross-platform behaviour: a workspace path with spaces and non-ASCII characters, and
...
- [ ] T040 Run the full gate from the repository root: `scripts/dev-tests.sh` plus
