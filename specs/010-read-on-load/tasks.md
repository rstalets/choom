...
- [ ] T001 Run `uv run pytest -q` and record the green baseline, so any failure later in this feature is
...
- [ ] T002 Add out-of-process mutation helpers to `tests/helpers.py`: create a document, complete a task,
...
- [ ] T003 [P] [US1] Integration test in `tests/integration/test_out_of_process_changes.py` covering the
...
- [ ] T004 [P] [US1] Integration test in `tests/integration/test_out_of_process_changes.py`: a malformed
...
- [ ] T005 [P] [US1] Integration test in `tests/integration/test_out_of_process_changes.py`: opening a
...
- [ ] T006 [US1] Update the helper at `tests/integration/test_daily_note_tui.py:16` to stop indexing
      `app.month_cache[("notes", …)]` and read through `app.visible_documents()` instead.
- [ ] T007 [US1] Update `tests/integration/test_mirror_reconcile_save.py::test_saving_without_touching_a_mirror_does_not_reload_tasks`
...
- [ ] T008 [P] [US1] Rename `cached_before` / `cached_after` at `tests/integration/test_mirror_propagation.py:195-206`
      to reflect that both are now fresh reads. Assertions are unchanged.

### Implementation for User Story 1

- [ ] T009 [US1] In `src/choom/tui/app.py`, make `visible_documents()` scan on every call: `scan_month` for
...
- [ ] T010 [US1] In `src/choom/tui/app.py`, make `visible_tasks()` call `load_tasks` on every call and
      `visible_warnings()` return the warnings from the read that produced the current rows.
- [ ] T011 [US1] In `src/choom/tui/app.py`, delete `month_cache`, `month_warnings`, `unfiled_cache`,
...
- [ ] T012 [US1] In `src/choom/tui/app.py`, delete `reload_tasks`, `refresh_document` and
      `_refresh_document_in`, plus the `refresh_document` call inside `toggle_task_and_track`.
- [ ] T013 [US1] In `src/choom/tui/app.py`, stop the three writers patching state: `_track_created` keeps
...
- [ ] T014 [US1] In `src/choom/tui/edit_screen.py`, delete the four refresh call sites at lines 104, 159,
...
- [ ] T015 [US1] In `src/choom/tui/list_screen.py`, change `_on_selected` (line ~451) to construct
...
- [ ] T016 [US1] In `src/choom/tui/list_screen.py`, have `refresh_rows` keep the warning count from its own
...
- [ ] T017 [US1] Verify no change is needed in `ListScreen.action_toggle_task` (line 305) or
...
- [ ] T018 [US1] Run `uv run pytest -q` and confirm the whole suite is green, including the six existing
...
- [ ] T019 [P] [US2] Unit test in `tests/unit/test_refresh_key.py` for the change-detection key: identical
      ...
      rendered.
- [ ] T020 [P] [US2] Integration test in `tests/integration/test_refresh_timer_tui.py`: invoking the tick
...
- [ ] T021 [P] [US2] Integration test in `tests/integration/test_refresh_timer_tui.py`: invoking the tick
...
- [ ] T022 [P] [US2] Integration test in `tests/integration/test_refresh_timer_tui.py`: the tick returns
      without reading or rendering when the command bar is open and when a filter is active (FR-012,
      FR-013, contract C4).
- [ ] T023 [P] [US2] Integration test in `tests/integration/test_refresh_timer_tui.py`: the interval is
...
- [ ] T024 [P] [US2] Performance test in `tests/performance/test_refresh_tick.py`: the read a tick performs
...
- [ ] T025 [US2] In `src/choom/tui/list_screen.py`, add `REFRESH_SECONDS = 2.0` as a module constant with a
      comment pointing at research R5 for why it is 2 s and not the issue's 10 s.
- [ ] T026 [US2] In `src/choom/tui/list_screen.py`, build the change-detection key from a read result:
...
- [ ] T027 [US2] In `src/choom/tui/list_screen.py`, implement the tick as **two methods** — one that
...
- [ ] T028 [US2] In `src/choom/tui/list_screen.py`, wire the tick: return early if the command bar is open
...
- [ ] T029 [US2] In `src/choom/tui/list_screen.py`, register the interval in `on_mount` via
...
- [ ] T030 [P] [US3] Integration test in `tests/integration/test_filter_hydration_tui.py`: typing a
...
- [ ] T031 [P] [US3] Integration test in `tests/integration/test_filter_hydration_tui.py`: closing the bar
...
- [ ] T032 [US3] Update `tests/integration/test_month_scope.py::test_filter_reads_each_month_at_most_once_per_session`
...
- [ ] T033 [P] [US3] Performance test in `tests/performance/test_filter_hydration.py`: on a 1,000-document
...
- [ ] T034 [US3] In `src/choom/tui/list_screen.py`, add a `@work(thread=True, exclusive=True, group="filter-hydrate")`
...
- [ ] T035 [US3] In `src/choom/tui/list_screen.py`, have `_on_filter_changed` (line 460, already `async`)
      `await` the worker before matching, so the first term matches the whole collection rather than a
      partial set (FR-017).
- [ ] T036 [US3] In `src/choom/tui/list_screen.py`, drop the worker handle in `_on_command_bar_closed`
      (line 527) so the snapshot's lifetime is exactly one bar session (FR-019).
- [ ] T037 [US3] Delete the unread `filter_loading` flag at `src/choom/tui/app.py:83`. `Worker.wait()`
...
- [ ] T038 Confirm the deletion is complete: `grep -rn "month_cache\|unfiled_cache\|fully_loaded\|reload_tasks\|refresh_document\|filter_loading" src/`
      returns nothing. The baseline count was 38 sites in `src/` (SC-008).
- [ ] T039 [P] Run `uv run ruff format --check .`, `uv run ruff check .` and `uv run mypy src tests`; fix
      anything the deletions left behind (unused imports of `ScanWarning`, `_read_document`, `Path` in
      `src/choom/tui/app.py` are likely).
- [ ] T040 [P] Verify cross-platform path handling is untouched by running
      `tests/integration/test_unicode_paths.py`; reading more often must not change how paths are built.
- [ ] T041 Walk [quickstart.md](./quickstart.md) end to end against a scratch workspace, including the
      two-terminal reproduction from issue #51.
- [ ] T042 Verify the TUI by hand on the target terminals listed in `docs/REQUIREMENTS.md`, watching
      specifically for flicker or scroll disturbance from the 2-second tick.
- [ ] T043 Review whether any user-facing documentation claims the TUI shows a snapshot; update `README.md`
      only if it does. No `AGENTS.md` change is expected — the workspace guidance never described caching.
- [ ] T044 Run the full suite once more and confirm CI is green on the PR before requesting review
