...
| # | Gate | Status |
|---|------|--------|
| I | All logic lands in `choom.core`; no I/O formatting, widget code, or argument parsing there. Core is testable without a terminal. **List the `core` functions this feature's reads and writes go through**, and justify any assembly done in an adapter that an existing `core` function already performs. | **PASS**, after correction. Core functions used: `scan_documents` (filter's whole-collection read), `scan_month` / `scan_unfiled` (scoped list loads), `list_months` (scope pane), `load_tasks`, `get_task`, `set_task_state`, `_read_document` (preview). No core function is added or changed. **This row originally read "PASS — no core change at all", which answers the wrong half of the gate**: the filter path was assembling the whole collection in the adapter, from `scan_month` per month plus `scan_unfiled`, when `scan_documents` already did it in one call and the CLI already called it. Caught after implementation, not at this gate; fixed, and the gate wording sharpened in `.specify/templates/plan-template.md` so the next plan is asked to list its core calls rather than assert an absence. |
