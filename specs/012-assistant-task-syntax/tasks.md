...
- [ ] T001 Run `scripts/dev-tests.sh` and record the green baseline, so any later failure is attributable
      to this feature
- [ ] T002 [P] Re-read `_finish_request` in `src/choom/tui/edit_screen.py` and confirm research R5 and R6
...
- [ ] T003 [P] Re-read `AssistantProfile.parse_reply` in `src/choom/core/assistants.py` and confirm the
...
- [ ] T004 Add `ReplyLine` (frozen, slots: `text: str`, `task: ParsedCommand | None`) and `ReplyCapture`
...
- [ ] T005 Add `"reply_capture_failed"` to the `ScanWarningReason` literal in
      `src/choom/core/models.py` (additive; no reader enumerates it exhaustively)
- [ ] T006 Re-export `ReplyLine`, `ReplyCapture`, `parse_reply_lines`, and `capture_reply_tasks` from
...
- [ ] T007 [P] [US1] Unit tests for the classifier's whole-line rule in `tests/unit/test_reply_lines.py`:
...
- [ ] T008 [P] [US1] Unit tests for the capture walk's success path in
...
- [ ] T009 [US1] Implement `parse_reply_lines(text: str) -> tuple[ReplyLine, ...]` in
...
- [ ] T010 [US1] Implement `capture_reply_tasks(workspace, text, *, source, source_id) -> ReplyCapture` in
...
- [ ] T011 [US1] Add `captures_tasks: bool` to `EditTarget` in `src/choom/tui/edit_screen.py`; set `True`
...
- [ ] T012 [US1] Call `capture_reply_tasks` from `_finish_request` in `src/choom/tui/edit_screen.py` —
      ...
      T011)
- [ ] T013 [US1] Seed `self._mirror_baseline[task.id] = False` for every task in `ReplyCapture.tasks` in
      `src/choom/tui/edit_screen.py`, at the same point `_capture_task` does it for a typed capture
      (FR-023, research R9)
- [ ] T014 [US1] Add `warn: bool = True` to `_render_status` in `src/choom/tui/edit_screen.py` and report
...
- [ ] T015 [US1] Add a `reply_with_tasks` mode to `_STUB_SOURCE` in `tests/conftest.py` printing prose and
      task lines interleaved, including one `/task.followup ... #tag` line
- [ ] T016 [US1] Integration test in `tests/integration/test_ai_command_tui.py` using
...
- [ ] T017 [P] [US2] Unit tests in `tests/unit/test_compose_prompt.py`: the clause present with
...
- [ ] T018 [US2] Add the `_TASK_SYNTAX` constant to `src/choom/core/assistants.py` — both forms, `#tags`
...
- [ ] T019 [US2] Add the required keyword-only `task_capture: bool` to `compose_prompt` in
      `src/choom/core/assistants.py`, appending `_TASK_SYNTAX` only when true (depends on T018)
- [ ] T020 [US2] Pass `task_capture=self.target.captures_tasks` from `_start_ai_request` in
...
- [ ] T021 [P] [US3] Unit tests for fence tracking in `tests/unit/test_reply_lines.py`: a backtick fence,
...
- [ ] T022 [US3] Add fence tracking to `parse_reply_lines` in `src/choom/core/editor_commands.py` per
...
- [ ] T023 [P] [US3] Add a `reply_explaining` mode to `_STUB_SOURCE` in `tests/conftest.py` printing an
      explanation with a fenced `/task` example and an inline mention
- [ ] T024 [US3] Integration test in `tests/integration/test_ai_command_tui.py` using `reply_explaining`:
...
- [ ] T025 [P] [US4] Unit test in `tests/unit/test_capture_reply_tasks.py`: a task captured from a reply
      and one captured by `capture_task` with the same words produce lines differing only in id and
      timestamp (FR-021, SC-003)
- [ ] T026 [US4] Extend the US1 integration test in `tests/integration/test_ai_command_tui.py`: complete
...
- [ ] T027 [P] [US4] Test in `tests/integration/test_ai_command_tui.py` that a second save straight after
...
- [ ] T028 [P] [US5] Unit tests for partial failure in `tests/unit/test_capture_reply_tasks.py`: one
      ...
      first
- [ ] T029 [US5] Add per-line recovery to `capture_reply_tasks` in `src/choom/core/mirrors.py`: catch
...
- [ ] T030 [US5] Compose the failure wordings in `src/choom/tui/edit_screen.py` per contracts §4:
      `N tasks captured; M could not be: <first reason>` with `⚠`, and the reason alone with `⚠` when
      everything failed (depends on T014, T029)
- [ ] T031 [US5] Integration test in `tests/integration/test_ai_command_tui.py`: with `tasks.md` made
...
- [ ] T032 [P] [US5] Test in `tests/integration/test_ai_command_tui.py` that a cancelled request whose
...
- [ ] T033 [P] Update the `/ai` bullet in `README.md` — what the assistant may now emit and what happens
      to it
- [ ] T034 [P] Update the inline task capture bullet in `README.md` — the same syntax is available to the
      assistant, and a reply's task lines are captured on identical terms
- [ ] T035 Confirm `src/choom/core/templates/AGENTS.md.tmpl` is deliberately unchanged and record why in
...
- [ ] T036 Run `scripts/dev-tests.sh` — full suite green on the baseline from T001, plus format, lint, and
      type checks
- [ ] T037 Walk [quickstart.md](./quickstart.md) by hand in a scratch workspace with a real assistant
