...
- [ ] T025 [US3] Implement `remove_discovery_files(*, keep: AssistantProfile | None = None) -> tuple[list[Path], list[str]]` in `src/choom/core/discovery.py`, deleting a file at a choom-owned path **only** when it contains the marker, and returning the paths removed plus a warning for each path left alone (FR-010, research R11)
- [ ] T026 [US3] Call it from `install_discovery_file` so a successful install leaves exactly one file across all assistants' locations (FR-008)
- [ ] T027 [US3] Handle `none` in the set branch of both adapters — `src/choom/cli/main.py` and `src/choom/tui/app.py` — removing every choom-owned file and succeeding when there is nothing to remove (FR-009)
- [ ] T028 [US3] Clear `launch_offer_made` on every explicit set of the assistant, in the same core call so neither adapter has to remember (FR-028)
- [ ] T029 [P] [US3] Add unit tests in `tests/unit/test_discovery_install.py`: repointing rewrites the path and leaves no trace of the previous workspace; switching assistants leaves exactly one file; `none` removes all; removal is idempotent; and a file at a choom-owned path **without** the marker is left on disk and reported as a warning
- [ ] T030 [P] [US3] Add a contract test in `tests/contract/test_config_assistant_cli.py` asserting the one-file invariant after any successful set
...
- [ ] T031 [US4] Report the discovery-file outcome on **stderr** from `_cmd_config_assistant` in `src/choom/cli/main.py` — path written, path removed, "no discovery file for `<name>`", or the failure and its reason — leaving stdout empty on a set (research R8, FR-014, FR-015)
- [ ] T032 [US4] Add `discovery_file` and `launch_offer_made` to the `--json` object and the two matching rows to the human-readable read output in `src/choom/cli/main.py`, renaming and removing nothing (FR-016, FR-033, `contracts/cli-config-assistant.md`)
- [ ] T033 [US4] Report the same outcomes in the TUI status bar from `handle_config_command` in `src/choom/tui/app.py`, in that interface's idiom (FR-015)
- [ ] T034 [US4] Confirm the exit code reports only the setting write: a discovery-file failure leaves it `0`, a rejected value `2`, a config that cannot be read or written `3` (FR-013)
- [ ] T035 [P] [US4] Add contract tests in `tests/contract/test_config_assistant_cli.py`: stdout empty on set; a read writes nothing anywhere; the four pre-existing `--json` keys keep their names and types; the unwritable-profile case records the setting, names the path on stderr, and exits `0`
