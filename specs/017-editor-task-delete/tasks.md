...
- [ ] T001 Run `scripts/dev-tests.sh` plus `uv run ruff format --check . && uv run ruff check . && uv run mypy src` from the repository root and confirm all green before touching anything. Record the test count so a later drop is visible
...
- [ ] T028 Run the full gate: `scripts/dev-tests.sh` plus `uv run ruff format --check . && uv run ruff check . && uv run mypy src`. Confirm green, confirm `tests/integration/test_delete_mirrors.py` still passes unchanged (the deliberate `choom task delete` asymmetry is intact), and confirm the diff touches no CLI file — `src/choom/cli/` must be untouched, which is the structural form of the plan's Principle II claim
